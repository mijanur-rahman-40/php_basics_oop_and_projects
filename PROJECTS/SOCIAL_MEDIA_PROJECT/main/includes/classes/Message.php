...
class Message
{
    ...
    public function getMostRecentUser()
    {
        ...
        // one result from the message table(getting first)
    ...
    public function sendMessage($user_to, $body, $date)
    {
        if ($body != "") {
            $userLoggedIn = $this->user_obj->getUsername();
            $query = mysqli_query($this->connection, "INSERT INTO messages (user_to,user_from,body,date,opened,viewed,deleted) VALUES('$user_to','$userLoggedIn','$body', '$date', 'no', 'no', 'no')");
        }
    }
    ...
    public function getMessages($otherUser)
    {
        ...
        while ($row = mysqli_fetch_array($get_messages_query)) {
            ...
            $button = "<span class='deleteButton' style='background: #ff5e50;border-radius: 3px;padding-left: 3px;padding-right: 4px' onclick='deleteMessage($id, this)'>X</span>";
    ...
    public function getLatestMessage($userLoggedIn, $user2)
    {
        ...
        $row = mysqli_fetch_array($query);
        $sent_by = ($row['user_to'] == $userLoggedIn) ? "He said: " : "You said: ";
