...
if (!isset($_POST['x']) && !isset($_FILES['image']['name'])) {
    ...
    $temp_path = 'assets/images/profile_pictures/' . $profile_id . '_temp.jpeg';
...
if (isset($_FILES['image']['name'])) {
    ...
    $upload_directory = $_SERVER['DOCUMENT_ROOT'] . '/SOCIAL_MEDIA_PROJECT/main/assets/images/profile_pictures';

    //Set File name
    $file_temp_name = $profile_id . '_original.' . md5(time()) . 'n' . $type; //the temp file name
    $full_path = $upload_directory . "/" . $file_temp_name; // the temp file path
    $file_name = $profile_id . '_temp.jpeg'; //$profile_id.'_temp.'.$type; // for the final resized image
    $full_path_2 = $upload_directory . "/" . $file_name; //for the final resized image

    //Move the file to correct location
    $move = move_uploaded_file($ImageTempName, $full_path);
    ...
    if (!$move) {
        ...
        $imgSrc = "assets/images/profile_pictures/" . $file_name; // the image to display in crop area
    ...
    clearstatcache();
    $original_size = getimagesize($full_path);
    ...
    if ($_FILES["image"]["type"] == "image/gif") {
        $src2 = imagecreatefromgif($full_path);
    } elseif ($_FILES["image"]["type"] == "image/jpeg" || $_FILES["image"]["type"] == "image/pjpeg") {
        $src2 = imagecreatefromjpeg($full_path);
    } elseif ($_FILES["image"]["type"] == "image/png") {
        $src2 = imagecreatefrompng($full_path);
    } else {
    ...
    // image destroy($full_path);
    @ unlink($full_path); // delete the original upload
...
if (isset($_POST['x'])) {
    ...
    //the image src
    $src = 'assets/images/profile_pictures/' . $_POST['src'];
    $final_name = $profile_id . md5(time());
    ...
    if ($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG') {
        ...
        imagejpeg($dst_r, "assets/images/profile_pictures/" . $final_name . "n.jpeg", 90);
    ...
    $result_path = "assets/images/profile_pictures/" . $final_name . "n.jpeg";

    //Insert image into database
    $insert_pic_query = mysqli_query($connection, "UPDATE users SET profile_picture='$result_path' WHERE username='$userLoggedIn'");
